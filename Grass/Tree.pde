class Tree {
   ...
   Tree() {
      ...
      Branch b = new Branch(new PVector(startX, height),new PVector(0,-2.0), treeHeight);
      b.isRoot = true;
   ...
   void draw() {
      ...
      // Apply perlin noise. Tempoorary disabled. Fix with Chris.
      // applyPerlin();
   }
   ...
   void split() {
      ...
      for (int i = 0; i < branches.size(); i++) {
         ...
         if (!b.isAnimating && b.isChild) {
            ...
            for (int j=0; j < n; j++) {
              Branch newB = b.branch(random(-45, 45));
              branches.add(newB);
            }
         }
      }
